...
def home_menu(*, is_admin: bool) -> InlineKeyboardMarkup:
    rows = [
        [
            InlineKeyboardButton("🟢 Server",       callback_data="nav_server"),
            InlineKeyboardButton("📈 Monitoring",   callback_data="nav_monitoring"),
        ],
        [
    ...
    if is_admin:
        rows.append([
            InlineKeyboardButton("⚙️ Operations", callback_data="nav_operations"),
...
def main_panel(*, is_admin: bool = True) -> InlineKeyboardMarkup:
    # Compatibility alias for existing handlers that still call main_panel().
...
def sysinfo_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("« Back",    callback_data="nav_monitoring")],
    ])
